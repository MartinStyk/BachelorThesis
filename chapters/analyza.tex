...
\subsection{ApkTool}
\label{ApkTool}
Nástroj na reverzné inžinierstvo Android aplikácií. Dokáže dekódovať zdroje aplikácie do takmer originálnej podoby. Do čitateľnej podoby prevádza súbory \zv{resources.arsc}, \zv{classes.dex} aj binárne XML súbory. Z~dekódovaných súborov umožňuje opätovné zostavenie APK súboru. Súbor \zv{classes.dex} je dekompilovaný do súborov vo formáte SMALI. Takéto súbory obsahujú nízkoúrovňový zdrojový kód. \zv{ApkTool} podporuje debugovanie smali kódu~\cite{apkTool}.
