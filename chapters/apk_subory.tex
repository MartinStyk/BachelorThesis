...
\subsection*{CERT.RSA}
\label{CERT.RSA} 
Súbor obsahujúci verejný kľúč vo forme \zv{X509} certifikátu, ktorý slúži na overenie digitálneho podpisu balíčka.
...
\begin{itemize}
\item Jazyk a región – jazyk je definovaný podľa kódovania \zv{ISO 639-1} s~možnosťou rozšírenia pomocou \zv{ISO 3166-1-alpha-2} regionálneho kódu. Napríklad obrázky špecifické pre zariadenia s~francúzskym jazykom sa nachádzajú v~priečinku \cesta{res/drawable-fr}
...
\item \bod{android:versionName} – informácia o~verzii aplikácie prezentovaná užívateľovo. Pre systém Android neposkytuje informáciu o~verzii, tú obsahuje atribút \zv{android:versionCode}.
...
Základnou hodnotou je \zv{normal}, ktorý reprezentuje povolenia s~nízkou mierou rizika. Tieto povolenia sú aplikácii automaticky schválené systémom Android počas inštalácie. Pre povolenia z~zvýšenou mierou rizika je určená hodnota \zv{dangerous}. Tieto povolenia typicky udeľujú aplikácii prístup k~citlivým dátam alebo k~prvkom zariadenia, ktoré môžu negatívne ovplyvniť jeho používanie. Pretože tento typ povolení prináša potenciálne riziko, systém ho nemôže udeliť automaticky, ale až po explicitnom súhlase používateľa.
